namespace QuadTree.Core
{
    ...
    public class QuadTree<T> where T : IBoundable
    {
        ...
        private static void GetCollisionCandidates(Node<T> node, Rectangle bounds, List<T> result)
        {
        ...
        private static void GetSubtreeContents(Node<T> node, Rectangle bounds, List<T> result)
        {
            if (node.Children != null)
            {
                foreach (var child in node.Children)
        ...
        private void ForEachDfs(Node<T> node, Action<List<T>, int, int> action, int depth = 1, int quadrant = 0)
        {
            ...
            if (node.Children != null)
            {
                foreach (var child in node.Children)
                {
                    this.ForEachDfs(child, action, depth + 1);
                }
            }
        }
    }
