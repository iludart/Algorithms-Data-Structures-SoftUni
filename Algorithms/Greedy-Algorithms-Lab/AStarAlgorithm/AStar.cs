namespace AStarAlgorithm
{
    ...
    public class AStar
    {
        ...
        public List<int[]> FindShortestPath(int[] startCoords, int[] endCoords)
        {
            ...
            {
                var currentNode = openNodesByFCost.ExtractMin();
                this.closedSet.Add(currentNode);

                if (currentNode == GetNode(endCoords[0], endCoords[1]))
                {
                    // found an exit
                }
        ...
        {
            return this.CalculateGCost(neighbour, GetNode(endCoords[0], endCoords[1]));
        }
        ...
        private List<Node> GetNeighbours(Node node)
        {
            ...
            for (int row = node.Row - 1; row <= node.Row + 1 && row < maxRow; row++)
            {
                ...
                for (int col = node.Col - 1; col <= node.Col + 1 && col < maxCol; col++)
                {
                    ...
                    if (row == node.Row && col == node.Col) continue;
